...
import java.io.{ByteArrayInputStream, ByteArrayOutputStream, FileInputStream, FileNotFoundException}
...
object EndpointContext {
  ...
  implicit class RichServerBuilder(builder: ServerBuilder) {
    def withContext(context: Option[EndpointContext]): ServerBuilder =
      context.foldLeft(builder) { case (b, httpsContext) =>
        b.enableHttps(httpsContext.connection)
      }
  }
}
