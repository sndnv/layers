package io.github.sndnv.layers.streaming
...
object Operators {
  ...
  implicit class ExtendedByteStringSource[+Mat](source: Source[ByteString, Mat]) {
    ...
    /**
      * Re-partitions the stream elements such that no individual element is above the specified size.
      *
      * @param withMaximumElementSize maximum element size
      * @return
      */
