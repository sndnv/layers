...
object MockJwksGenerator {
  ...
  def generateRandomRsaKeys(count: Int, keyPrefix: Option[String]): Seq[RsaJsonWebKey] =
    (0 until count).map { i =>
      generateRandomRsaKey(keyPrefix.map(prefix => s"$prefix-${i.toString}"))
    }

  def generateRandomEcKeys(count: Int, keyPrefix: Option[String]): Seq[EllipticCurveJsonWebKey] =
    (0 until count).map { i =>
      generateRandomEcKey(keyPrefix.map(prefix => s"$prefix-${i.toString}"))
    }

  def generateRandomSecretKeys(count: Int, keyPrefix: Option[String]): Seq[OctetSequenceJsonWebKey] =
    (0 until count).map { i =>
      generateRandomSecretKey(keyPrefix.map(prefix => s"$prefix-${i.toString}"))
    }
