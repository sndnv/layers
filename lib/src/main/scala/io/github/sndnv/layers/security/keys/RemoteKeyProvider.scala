...
final class RemoteKeyProvider(
  ...
  private val cache = MemoryStore[String, Key](name = "jwk-provider-cache")
...
object RemoteKeyProvider {
  def apply(
    ...
    refreshRetryInterval: FiniteDuration,
    issuer: String
  )(implicit system: ActorSystem[Nothing], telemetry: TelemetryContext, timeout: Timeout): RemoteKeyProvider =
