...
object MemoryStore {
  ...
  /**
    * Creates a new in-memory key-value store with the specified `name`.
    *
    * Note: The data store can be initialized ([[MemoryStore.init]]) and
    * reset ([[MemoryStore.drop]]) as many times as needed.
    *
