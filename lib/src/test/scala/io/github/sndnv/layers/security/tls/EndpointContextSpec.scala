package io.github.sndnv.layers.security.tls
...
class EndpointContextSpec extends UnitSpec {
  "An EndpointContext" should "load store config" in {
    val expectedConfig = EndpointContext.StoreConfig(
      storePath = "./lib/src/test/resources/certs/localhost.p12",
      storeType = "PKCS12",
      storePassword = ""
    )
  ...
  it should "create connection contexts (server/client)" in {
    import EndpointContext.RichServerBuilder
    ...
    val _ = Http()
      ...
      .withContext(context = Some(serverContext))
      .bindFlow(
        handlerFlow = get {
  ...
  it should "fail if a store is missing" in {
    val expectedMessage = "Store [./lib/src/test/resources/certs/missing.p12] with type [JKS] was not found"
  ...
  private val config: Config = ConfigFactory.load().getConfig("io.github.sndnv.layers.test.security.tls")

  private val ports: mutable.Queue[Int] = (13000 to 13100).to(mutable.Queue)

  private implicit val system: ActorSystem[Nothing] = ActorSystem(
    guardianBehavior = Behaviors.ignore,
    name = "EndpointContextSpec"
  )
}
